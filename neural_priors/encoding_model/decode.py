...
def main(subject, range_n, smoothed, bids_folder, gaussian, n_voxels=100, roi='NPCr'):
    ...
    if range_n != 'both':
        behavior = behavior[behavior.range == range_n]
        betas = betas.loc[behavior.index]
    ...
    for (test_session, test_run), test_behavior in behavior.groupby(['session', 'run']):
        
        test_data = betas.loc[(slice(None), test_session, test_run, slice(None)), :].copy()
        train_data = betas[~((betas.index.get_level_values('session') == test_session) & (betas.index.get_level_values('run') == test_run))].copy()

        test_paradigm = test_behavior['n']

        train_behavior = behavior[~((behavior.index.get_level_values('session') == test_session) & (behavior.index.get_level_values('run') == test_run))].copy()
        train_paradigm = train_behavior['n']
    ...
    pdf = pd.concat(pdfs, keys=keys, names=['session', 'run'])

    pdf.to_csv(op.join(target_dir, f'sub-{subject}_desc-decoding_within.pdfs.tsv'), sep='\t')
