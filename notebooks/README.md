...
| `all_likelihoods.tsv` | regenerate (~16 MB, needs all 14 main models) | `archive/encoding_model/analyze_likelihoods.ipynb` → `derivatives/encoding_models/all_likelihoods.tsv`; copy here. NB: that archived notebook predates the 2026-03-03 `calc_likelihood.py` output change (loglikelihood NIfTIs instead of likelihood TSVs) — update its glob before rerunning, or rebuild from the `loglikelihood` column in the extracted-pars TSVs |
